...
const ActualizarCostoME: React.FC = () => {
  ...
  const fetchDefaultValue = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_API_URL}/tabla-config/matriz_energia?planta=${plantaParaEscritura}`,
        {
          headers: {
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      toast({
        title: 'Actualizado',
        description: `Costo de energía de ${plantaLabel} actualizado a $${numericValue.toFixed(2)}. Se recalculó la matriz energética de esa planta.`,
      });
            ...
            <CardContent className="pt-6">
              <div className="flex items-start gap-3 text-orange-800">
                ...
                <div className="text-sm">
                  Estás en modo <strong>"Ambas plantas"</strong>. Para editar el costo de energía cambiá el selector del header a <strong>Catamarca</strong> o <strong>Varela</strong>.
                </div>
              </div>
            </CardContent>
          </Card>
        ...
        <Card className="max-w-md mx-auto">
          <CardHeader>
            <CardTitle className="flex items-center space-x-2">
              <span>Actualizar costo de energía {plantaParaEscritura ? `(${plantaLabel})` : ''}</span>
            </CardTitle>
          </CardHeader>
          ...
          <CardContent className="space-y-6">
            <div className="space-y-2">
              ...
              <p className="text-sm text-muted-foreground">
                Este valor se aplica a todas las filas de la matriz energética de <strong>{plantaLabel}</strong>. No afecta a la otra planta.
              </p>
            </div>
