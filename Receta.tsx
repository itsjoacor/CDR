...
const Receta: React.FC = () => {
  ...
  // Usa el endpoint bulk /recetas-normalizada/flags/cdr-cero (1 sola request).
  // Si el endpoint no está disponible, hace fallback al modo legacy (N requests).
  const verificarTodosLosCDR = async (productosList: Producto[]) => {
    try {
      const res = await fetch(
        `${import.meta.env.VITE_API_URL}/recetas-normalizada/flags/cdr-cero`,
        { headers: { Authorization: `Bearer ${token}` } }
      );

      if (res.ok) {
        const arr: { codigo_producto: string }[] = await res.json();
        const results: CdrCeroMap = {};
        productosList.forEach((p) => {
          results[p.codigo_producto] = false;
        });
        arr.forEach((r) => {
          results[r.codigo_producto] = true;
        });
        setCdrCeroMap(results);
        return;
      }
    } catch (error) {
      console.error('Error al obtener flags CDR cero (bulk):', error);
    }

    // Fallback: comportamiento original (N requests en lotes)
    const results: CdrCeroMap = {};
    const CHUNK_SIZE = 10;
  ...
  useEffect(() => {
    const fetchAll = async () => {
      try {
        ...
        // Disparar las 3 requests en paralelo
        const [resProd, resZero, resCdr] = await Promise.all([
          fetch(`${import.meta.env.VITE_API_URL}/productos`, { headers }),
          fetch(
        ...
        // Flags CDR cero — usa la respuesta bulk si está OK, si no fallback al modo legacy
        if (resCdr.ok) {
          const cdrArr: { codigo_producto: string }[] = await resCdr.json();
          const cdrMap: CdrCeroMap = {};
          prodData.forEach((p) => {
            cdrMap[p.codigo_producto] = false;
          });
          cdrArr.forEach((r) => {
            cdrMap[r.codigo_producto] = true;
          });
